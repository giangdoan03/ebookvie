...
// Xu ly phan tai file
function handle_ajax_download_link() {
    if (isset($_POST['product_id']) && isset($_POST['field_name'])) {
        $product_id = intval($_POST['product_id']);
        $field_name = sanitize_text_field($_POST['field_name']);
        $download_link = get_field($field_name, $product_id);
...
add_action('wp_ajax_get_download_link', 'handle_ajax_download_link');
add_action('wp_ajax_nopriv_get_download_link', 'handle_ajax_download_link');
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                downloadLinks.forEach(function(link) {
                    link.addEventListener('click', function(event) {
                        ...
                        let countdown = 5;
                        link.textContent = `Vui lòng chờ ${countdown}s`;
                        ...
                        const countdownInterval = setInterval(() => {
                            ...
                            if (countdown === 0) {
                                ...
                                link.textContent = `Tải xong ${fieldName.replace('link', 'link ')}`;
                            }
                        }, 1000);
                        
                        const data = new FormData();
                        data.append('action', 'get_download_link');
...
function woo_rename_tabs( $tabs ) {
  $tabs['description']['title'] = __( 'Giới thiệu về eBook' );        // Rename the description tab
  return $tabs;
}
...
add_filter('wp_get_attachment_image_attributes', 'add_alt_text_to_product_images', 10, 2);

function add_alt_text_to_product_images($attr, $attachment) {
    if (is_product()) {
        global $product;
        if (!$product) {
...
function add_custom_product_meta_box() {
    add_meta_box(
        'custom_product_meta_box', // ID của meta box
        __('Số lượt xem sản phẩm', 'woocommerce'), // Tiêu đề của meta box
        'display_custom_product_meta_box', // Hàm hiển thị nội dung của meta box
        'product', // Post type
        'side', // Vị trí của meta box
        'default' // Độ ưu tiên hiển thị của meta box
    );
}
add_action('add_meta_boxes', 'add_custom_product_meta_box');

// Hiển thị nội dung của meta box
function display_custom_product_meta_box($post) {
    $count_key = 'post_views_count';
    $post_views_count = get_post_meta($post->ID, $count_key, true);

    wp_nonce_field(basename(__FILE__), 'custom_product_nonce'); // Bảo mật với nonce
    ?>
    <p>
        <label for="post_views_count"><?php _e('Số lượt xem:', 'woocommerce'); ?></label>
        <input type="number" name="post_views_count" id="post_views_count" value="<?php echo esc_attr($post_views_count); ?>" />
    </p>
    <?php
}
...
function save_custom_product_meta_box($post_id) {
    ...
    if (!isset($_POST['custom_product_nonce']) || !wp_verify_nonce($_POST['custom_product_nonce'], basename(__FILE__))) {
        return $post_id;
    }
    ...
    // Lưu dữ liệu
    $count_key = 'post_views_count';
    $post_views_count = isset($_POST['post_views_count']) ? intval($_POST['post_views_count']) : '';
    update_post_meta($post_id, $count_key, $post_views_count);
}
...
function __codfe_search_by_title_only( $search, $wp_query ) {
    ...
    // Xác định điều kiện khớp từ khóa
    $n = ! empty( $q['exact'] ) ? '' : '%';
    $search = '';
    $searchand = '';
    ...
    // Nếu có điều kiện tìm kiếm, thêm điều kiện vào truy vấn SQL
    if ( ! empty( $search ) ) {
        $search = " AND ({$search}) ";
...
add_action( 'phpmailer_init', function( $phpmailer ) {
    ...
    $phpmailer->Password   = 'mzsw obnc glww xtnh'; //điền mật khẩu ứng dụng mà bạn đã tạo ở trên
...
add_filter('rank_math/frontend/title', 'custom_doc_sach_title', 10, 1);

function custom_doc_sach_title($title) {
    if (is_page('doc-sach')) {
        // Lấy giá trị của biến query 'ten_sach'
        $slug = get_query_var('ten_sach');
        if ($slug) {
            // Tìm bài viết dựa trên slug 'ten_sach'
            $args = array(
                'name'        => $slug,
            ...
            if ($product_posts) {
                ...
                $title = 'Đọc Online Sách ' . $product_title;
            }
        }
    }
    return $title;
}
// xoá thẻ canonical và og:url tùy chỉnh chỉ trên trang /doc-sach/url
add_filter('rank_math/frontend/canonical', 'custom_remove_canonical_on_doc_sach');

function custom_remove_canonical_on_doc_sach($canonical) {
    if (is_page('doc-sach') && get_query_var('ten_sach')) {
        return false;
    }
    return $canonical;
}

// Thêm thẻ canonical và og:url tùy chỉnh chỉ trên trang /doc-sach/url
add_action('wp_head', 'custom_head_tags_on_doc_sach');

function custom_head_tags_on_doc_sach() {
    if (is_page('doc-sach') && get_query_var('ten_sach')) {
        $slug = get_query_var('ten_sach');
        if ($slug) {
            $product_url = home_url('/doc-sach/' . $slug);
            echo '<link rel="canonical" href="' . esc_url($product_url) . '">' . "\n";
        }
    }
}

//CODE HIEN THI mota cho trang docsach
    ...
    if (is_page('doc-sach')) {
        ...
        if ($slug) {
            ...
            if ($product_posts) {
                ...
                // Lấy tên của taxonomy 'tac-gia'
                $terms = get_the_terms($product_post->ID, 'tac-gia');
                if ($terms && !is_wp_error($terms)) {
                    $tac_gia_names = wp_list_pluck($terms, 'name');
                    $tac_gia_name = implode(', ', $tac_gia_names);
                    
                    // Tạo thẻ meta description
                    $description = 'Đọc sách ' . $product_title . ' của tác giả ' . $tac_gia_name;
...
// Thêm .epub vào các loại MIME được phép tải lên
...
// Thay đổi thư mục tải lên cho các file .epub
function custom_upload_dir($dirs) {
    // Kiểm tra nếu tệp hiện tại đang được tải lên là tệp EPUB
    if (isset($_FILES['async-upload']) && $_FILES['async-upload']['name']) {
        $file = $_FILES['async-upload']['name'];
        $file_ext = pathinfo($file, PATHINFO_EXTENSION);

        // Nếu tệp là EPUB, thay đổi thư mục tải lên
        if ($file_ext == 'epub') {
            $dirs['path'] = $dirs['basedir'] . '/epub';
            $dirs['url'] = $dirs['baseurl'] . '/epub';
            $dirs['subdir'] = '/epub';
        }
    }
